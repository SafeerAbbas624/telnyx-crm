...
export const funderRequirements: Record<string, DocumentRequirement[]> = {
  ...
  'roc capital': rocRequirements,
  roc360: rocRequirements,
  ahl: ahlRequirements,
  'american heritage lending': ahlRequirements,
  velocity: velocityRequirements,
  // Legacy names that might be in old data
  'wells fargo': baseRequirements,
  'chase bank': baseRequirements,
  'bank of america': baseRequirements,
  'quicken loans': baseRequirements,
};

export function getRequirementsForFunder(funder: string): DocumentRequirement[] {
  const lenderKey = funder.toLowerCase().trim();
  return funderRequirements[lenderKey] || baseRequirements;
}
...
export function getRequiredDocumentCount(funder: string): number {
  const requirements = getRequirementsForFunder(funder);
  return requirements.filter(req => req.required).length;
}

export function getFunderSpecificDocumentCount(funder: string): number {
  const requirements = getRequirementsForFunder(funder);
  return requirements.filter(req => req.funderSpecific).length;
}
