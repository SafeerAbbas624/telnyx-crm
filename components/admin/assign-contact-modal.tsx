...
import { UserPlus, Users, Phone, Mail, CheckCircle, User } from "lucide-react"
...
export default function AssignContactModal({ 
  contact,
  contacts,
  onAssignmentComplete, 
  trigger,
  buttonText = "Assign Contact",
  ...
  // Determine which contacts to work with
  const contactsToAssign = contacts || (contact ? [contact] : [])
  ...
  const handleAssignContacts = async () => {
    ...
    if (contactsToAssign.length === 0) {
      toast({
        title: 'Error',
        description: 'No contacts to assign',
        variant: 'destructive',
      })
      return
    }
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: `Successfully assigned ${contactsToAssign.length} contact(s) to ${selectedUsers.length} team member(s)`,
      })
  ...
  if (contactsToAssign.length === 0) {
    return null // Don't render if no contacts to assign
  }
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Assign Contact{contactsToAssign.length > 1 ? 's' : ''} to Team
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="p-3 bg-blue-50 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-blue-900">
                {contactsToAssign.length} Contact{contactsToAssign.length !== 1 ? 's' : ''} Selected
              </span>
            </div>
            <div className="text-xs text-blue-700">
              {contactsToAssign.slice(0, 3).map(contact => 
                `${contact.firstName} ${contact.lastName}`
              ).join(', ')}
              {contactsToAssign.length > 3 && ` and ${contactsToAssign.length - 3} more`}
            </div>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            ...
            >
              ...
              Assign Contact{contactsToAssign.length > 1 ? 's' : ''}
            </Button>
          </div>
        </div>
      </DialogContent>
