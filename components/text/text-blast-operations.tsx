...
export default function TextBlastOperations({ onSwitchTab }: TextBlastOperationsProps) {
  ...
  const fetchBlasts = useCallback(async () => {
    try {
      const res = await fetch('/api/text-blast?limit=50')
      if (res.ok) {
  ...
  const fetchActiveCount = useCallback(async () => {
    try {
      const res = await fetch('/api/text-blast/kill-all')
      if (res.ok) {
        const data = await res.json()
        setActiveCount(data)
      }
  ...
  const pauseBlast = async (id: string) => {
    try {
      const res = await fetch(`/api/text-blast/${id}/pause`, { method: 'POST' })
      if (res.ok) {
        toast.success('Blast paused')
        loadData()
      } else {
  ...
  const resumeBlast = async (id: string) => {
    try {
      const res = await fetch(`/api/text-blast/${id}/resume`, { method: 'POST' })
      if (res.ok) {
  ...
  const cancelBlast = async (id: string) => {
    try {
      const res = await fetch(`/api/text-blast/${id}/cancel`, { method: 'POST' })
      if (res.ok) {
  ...
  const deleteBlast = async (id: string) => {
    try {
      const res = await fetch(`/api/text-blast/${id}`, { method: 'DELETE' })
      if (res.ok) {
  ...
  return (
    <div className="h-full flex flex-col gap-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            ...
            Text Blast Operations
          </h2>
          <p className="text-muted-foreground">Monitor and control all text blast campaigns</p>
        </div>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle className="flex items-center gap-2 text-red-600">
                  ...
                  Emergency Stop All Blasts
                </AlertDialogTitle>
                <AlertDialogDescription asChild>
                  <div>
                    <div>This will immediately stop ALL running text blast operations:</div>
      ...
      <Card className="flex-1">
        <CardHeader>
          <CardTitle>Recent Text Blasts</CardTitle>
          <CardDescription>Last 50 text blast campaigns</CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <div key={blast.id} className="border rounded-lg p-4 hover:bg-muted/50">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground line-clamp-1">{blast.message}</p>
