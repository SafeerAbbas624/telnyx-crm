...
function generateDocumentResponse(
  ...
  if (!existingDocCategories.has("borrower_entity")) {
    missingCategories.push("borrower_entity");
  }
  ...
  if (missingCategories.length > 0) {
    ...
    missingCategories.forEach(category => {
      response += `- ${category.charAt(0).toUpperCase() + category.slice(1).replace('_', ' ')}\n`;
      
      if (category === "borrower_entity") {
        response += "  (Driver's License, Entity Documents, Operating Agreement)\n";
    ...
    return {
      content: "Here's a standard document checklist for DSCR loans:\n\n" +
        "**Borrower Documents:**\n" +
        dscrLoanKnowledge.documents.required.map(doc => `- ${doc}`).join("\n") + 
        "\n\n**Insurance Documents:**\n" +
        dscrLoanKnowledge.documents.insurance.map(doc => `- ${doc}`).join("\n") +
        "\n\n**Title Documents:**\n" +
        dscrLoanKnowledge.documents.title.map(doc => `- ${doc}`).join("\n")
    };
  }
...
function generateProcessResponse(
  ...
  const nextSteps = dscrLoanKnowledge.processes[stage as keyof typeof dscrLoanKnowledge.processes];
  ...
  if (query.includes("title") || query.includes("commitment")) {
    return {
      content: "Here's a template for requesting a title commitment:\n\n" + 
        dscrLoanKnowledge.templates.title_agent_email
    };
  }
  ...
  if (query.includes("insurance")) {
    return {
      content: "Here's a template for requesting insurance information:\n\n" + 
        dscrLoanKnowledge.templates.insurance_request_email
    };
  }
...
function generateDSCRInfoResponse(query: string): FallbackResponse {
  ...
  for (const [key, value] of Object.entries(dscrLoanKnowledge.common_questions)) {
...
function generateGeneralLoanResponse(
  ...
  const propertyAddress = property?.address || loan.propertyAddress || "the property";
  const propertyCity = property?.city || loan.propertyCity || "";
  const propertyState = property?.state || loan.propertyState || "";
  
  // Provide a summary of the loan file
  return {
    content: `This is a ${loan.loanType || "DSCR"} loan for ${propertyAddress}${propertyCity ? `, ${propertyCity}` : ''}${propertyState ? `, ${propertyState}` : ''}. The borrower is ${loan.borrowerName} and the loan amount is $${loan.loanAmount?.toLocaleString() || "not yet specified"}.\n\n` +
      `There are currently ${tasks.filter((t: any) => !t.completed).length} open tasks on this file. The loan status is ${loan.dealStage || "In Process"}.\n\n` +
